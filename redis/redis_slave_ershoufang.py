...
class Mongo(object):
    ...
    def MongoInit(self):
        ershoufang_db =self.db["ershoufang"]
        print"******MongoInit() success*****"
        return ershoufang_db



class Redis_Ershoufang(object):
    ...
    def grab_for_detail(self,page):
        ...
        if(len(house_name)>0):
            ...
            print thread.getName(),house_name[0],rooms,area,house_price,unitPriceValue
    ...
    def start_work(self):
        ...
        while True:
            ...
            ershou_url = self.r.spop('ershoufang_urls')
            ...
            res=self.grab_orign(ershou_url,proxy)
            if res=="None_page":
                proxy_is_good=False
                self.r.sadd('ershoufang_urls',ershou_url)#没有得到请求页面的要上交给master
                print t_name,'res=="None_page",then give it back to redis’'
                continue
            else:
                house_name=self.grab_for_detail(res)
                if house_name=="None_house_name":
                    self.r.sadd('ershoufang_urls',ershou_url)#没有正确解析的要上交给master
...
if __name__ == '__main__':
    ...
    Mongodb=Mongo()
    ershoufang_db=Mongodb.MongoInit()


    Redis_Obj=Redis_Ershoufang()
